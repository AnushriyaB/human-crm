...
import { User, MapPin, Globe, Users, Handshake, ChevronDown, Search, Camera, Copy, Check, Eye, EyeOff } from 'lucide-react';
...
export default function CoreIdentityCard({ friend, isEditing, onUpdate }) {
    ...
    const handleCountryChange = (country) => {
        const updates = {
            ...
            location: country.name, // Set location to country name
        };
        // Auto-set timezone if country has a single timezone
        if (country.tz !== 'Multiple') {
            updates.timezone = country.tz;
            // For countries without states, geocode the country directly
    ...
    const handleStateChange = (state) => {
        ...
        // Trigger geocoding with the new location directly
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-start gap-5">
                    ...
                    <div className="relative group">
                        <div className="w-20 h-20 rounded-2xl bg-[var(--color-bg-secondary)] flex items-center justify-center overflow-hidden shrink-0 border-2 border-[var(--color-border)]">
                            {friend.photo ? (
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <div>
                            ...
                            {isEditing ? (
                                ...
                                <p className="text-sm font-medium">{friend.nickname || <span className="text-[var(--color-text-secondary)] italic">Not set</span>}</p>
                            )}
                        </div>
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <label className={labelClass.replace(' mb-2 block', '')}>Connection</label>
                        </div>
                        {isEditing ? (
                            <select
                            ...
                            <p className="text-sm font-medium capitalize">{friend.relationshipType || <span className="text-[var(--color-text-secondary)] italic">Not set</span>}</p>
                        )}
                    </div>
                </div>
                ...
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <Handshake size={14} className="text-[var(--color-text-secondary)]" />
                        <label className={labelClass.replace(' mb-2 block', '')}>How You Met</label>
                    </div>
                    {isEditing ? (
                        <input
                            type="text"
                        ...
                        <p className="text-sm font-medium">{friend.howMet || <span className="text-[var(--color-text-secondary)] italic">Not set</span>}</p>
                    )}
                </div>
                            ...
                            <button
                                ...
                                className={`
                                    ...
                                    ${copied
                                        ...
                                        : 'bg-[var(--color-bg-secondary)] hover:bg-[var(--color-brand)]/10 text-[var(--color-text-secondary)] hover:text-[var(--color-brand)] border border-[var(--color-border)]'
                                    }
                                `}
                            >
                                {copied ? (
